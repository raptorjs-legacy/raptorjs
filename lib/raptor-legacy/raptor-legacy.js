(function () {
    /*jshint strict:false */
    var raptor = require('raptor'), legacyRaptor, global = raptor.global, logging = require('raptor/logging'), amdDefine = global.define || raptor.createDefine();
    //Create a new global raptor object
    global.raptor = legacyRaptor = raptor.extend({}, raptor);
    var legacyNamesRE = /^(arrays|json.*|debug|listeners|loader.*|locale.*|logging|pubsub|objects|strings|templating.*|widgets)$/, normalize = function (id) {
        ...
        }, createLoggerFunction = function (name) {
            //Helper function invoked for each class to add a "logger()" function to the class prototype
            var _logger;
            return function () {
                return _logger ? _logger : _logger = logging.logger(name);
            };
        }, _define = function (args, isClass, isEnum, isMixin) {
            var last = args.length - 1, factory = args[last], name = args[0];
            if (typeof args[0] != 'string') {
                name = '(anonymous)';
            }
            if (typeof factory == 'function') {
                args[last] = function () {
                    var o = factory(legacyRaptor), mixinsTarget = o;
    ...
    raptor.extend(legacyRaptor, {
        require: function (ids, callback, thisObj, ignoreMissing) {
            return ignoreMissing ? legacyRaptor.find(ids) : raptor.require(normalizeIds(ids), callback, thisObj);
        },
        find: find,
        load: find,
        define: defineModule,
