define.Class('raptor/optimizer/PageBundles', ['raptor'], function (raptor, require, exports, module) {
    'use strict';
    ...
    PageBundles.prototype = {
        build: function () {
            var optimizer = require('raptor/optimizer'), bundleMappings = this.bundleMappings,
                // array of all asynchronous package manifests
                asyncPackages = [], deferred = promises.defer(), isDone = false, _this = this;
            function done() {
                if (isDone) {
                    return;
                }
            ...
            // TODO: Modify the bundling process so that this step can be externalized
            // (localization should not be core to raptor)
                        ...
                        if (result) {
                            ...
                            if (result.packages) {
                                result.packages.forEach(function (i18nPackage) {
                                    ...
                                    logger.info('Added i18n package "' + i18nPackage.name + '". Asynchronous: ' + !!i18nPackage.async);
                                });
                            }
                        }
                        deferred.resolve();
            ...
            function buildAsyncPageBundles() {
                var promise = optimizer.forEachDependency({
                        ...
                        handleDependency: function (dependency, context) {
                            ...
                            var bundleKey = bundle.getKey();
                            if (!this.bundleLookup[bundleKey]) {
            ...
            promise.then(buildAsyncPageBundles).fail(onError);
