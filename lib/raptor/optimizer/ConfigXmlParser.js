define.Class('raptor/optimizer/ConfigXmlParser', ['raptor'], function (raptor, require) {
    'use strict';
    var BundleConfig = require('raptor/optimizer/BundleConfig'), BundleSetConfig = require('raptor/optimizer/BundleSetConfig'), PageConfig = require('raptor/optimizer/PageConfig'), strings = require('raptor/strings'), files = require('raptor/files'), File = require('raptor/files/File');
    var ConfigXmlParser = function () {
    };
    ConfigXmlParser.prototype = {
        parse: function (xml, configFilePath, config) {
            var configDir = new File(configFilePath).getParent(), configResource = require('raptor/resources').createFileResource(configFilePath);
            ...
            var objectMapper = require('raptor/xml/sax/object-mapper'), reader, dependencyHandler = {
                    _type: 'object',
                    ...
                    '<bundle>': {
                        ...
                        'raptor-no-conflict': {
                            _type: 'boolean',
                            _set: function (bundleConfig, name, value) {
                                if (value === false) {
                                    bundleConfig.disableWrapper('raptor-no-conflict');
                                } else {
                                    // NOTE: undefined is treated as true to support <raptor-no-conflict />
                                    bundleConfig.enableWrapper('raptor-no-conflict');
                                }
                            }
                        },
                        '<*>': dependencyHandler
                    ...
                    '<plugin>': {
                        ...
                        '<config>': {
                            _type: 'object',
                            _begin: function (plugin) {
                                return plugin;
                            },
                            '*': {
                                _type: 'string',
                                _set: function (plugin, name, value) {
                                    if (value === 'true') {
                                        value = true;
                                    } else if (value === 'false') {
                                        value = false;
                                    } else if (value.test(/^[0-9]+$/)) {
                                        value = parseInt(value, 10);
                                    }
                    ...
                    'raptor-no-conflict': {
                        ...
                        _set: function (config, name, value) {
                            if (value === false) {
                                config.disableBundleWrapper('raptor-no-conflict');
                            } else {
                                // NOTE: undefined is treated as true to support <raptor-no-conflict />
                                config.enableBundleWrapper('raptor-no-conflict');
                            }
                        }
                    },
                    ...
                    '<pages>': {
                        '<page>': {
                            ...
                            'package-manifest': {
                                _type: 'string',
                                _set: function (parent, name, value) {
                                    var packagePath = resolvePath(value), packageResource;
                            ...
                            '<dependencies>': {
                                ...
                                _end: function (dependencies, pageConfig) {
                                    var packageManifest = require('raptor/packaging').createPackageManifest({ dependencies: dependencies.dependencies }, configResource);
            ...
            reader.read(xml, configFilePath);
            if (!config.outputDir) {
