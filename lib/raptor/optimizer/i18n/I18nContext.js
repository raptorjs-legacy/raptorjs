...
define.Class('raptor/optimizer/i18n/I18nContext', function(require, exports, module) {
    ...
    I18nContext.prototype = {
        ...
        _readDictionaryResource: function(i18nResource, localeCode, callback, thisObj) {
            ...
            i18n._readRawDictionary(i18nResource, localeCode, function(err, dictionary) {
                if (err) {
         ...
         * For example, if "en_US" is missing a translation then this method will try
         * to find the next best translation by searching "en" and then default locale.
         */
        _rollUpDictionaries: function(i18nResource) {
            ...
            for (var i = 0; i < this._locales.length; i++) {
                var localeCode = this._locales[i],
                    dictionary = this.getDictionary(dictionaryName, localeCode);
                ...
                parentDictionary = (localeCode.charAt(2) === '_') ?
                    this.getDictionary(dictionaryName, localeCode.substring(0, 2)) :
                    undefined;
        ...
        _readI18nDependency: function(dependency, callback) {
            ...
            var i18nResource = dependency.getI18nResource(),
            ...
            function onReadDictionary(err) {
                ...
                if (pending === 0) {
                    if (errors) {
                        ...
                        this._rollUpDictionaries(i18nResource);
                        callback(null);
                    }
                }
            }
        ...
        buildBundles: function(pageBundles, callback) {
            ...
            for (i = 0; i < locales.length; i++) {
                ...
                var packageManifest = packaging.createPackageManifest({
                    ...
                    // the "async" property is used as an indicator to the builder to determine if this i18n module should be
                    // included asynchronously or synchronously
                    async: async,
                ...
                var bundle = bundleMappings.addDependencyToBundle(packageManifest.getDependencies()[0], bundleName);
                if (!async) {
                    // If the bundle is not asynchronous then add bundle to the JavaScript slot so that
                    // it gets compiled into the bundle output file.

                    // TODO: How should we pick a slot for the i18n module?
                    bundle.slot = 'body';
