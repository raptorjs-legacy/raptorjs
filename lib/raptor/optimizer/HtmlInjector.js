define.Class('raptor/optimizer/HtmlInjector', ['raptor'], function (raptor, require) {
    'use strict';
    var startRegExp = /<!--\s*\[\s*raptor-dependency\:?\s+(\w+)\s*\]\s*-->/g, endRegExp = /<!--\s*\[\/\s*raptor-dependency\s*\]\s*-->/g;
    ...
    HtmlInjector.prototype = {
        findSlots: function (pageHtml) {
            var injectIndexes = this.injectIndexes, parts = this.parts, startMatches, endMatch, begin = 0;
