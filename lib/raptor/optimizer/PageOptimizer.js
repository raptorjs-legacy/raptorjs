define.Class(
    ...
    function(raptor, require, exports, module) {
        ...
        PageOptimizer.prototype = {
            ...
            buildPageBundlesAsync: function(options) {
                return this.buildPageBundles(options, true);
            },

            buildPageBundles: function(options, async) {
                ...
                var bundleMappings = config.isBundlingEnabled() ? 
                        this.getBundleMappingsCached(bundleSetConfig, enabledExtensions, async) : //Only load the bundle mappings if bundling is enabled
                        null;
                ...
                function buildPageBundles(bundleMappings) {
                    ...
                    return pageBundles.build(async);
                }
                ...
                if (async) {
                    return bundleMappings.then(function(bundleMappings) {
                        var pageBundlesPromise = buildPageBundles(bundleMappings);
                        pageBundlesPromise.then(done);
                        return pageBundlesPromise;
                    })
                }
                else {
                    var pageBundles = buildPageBundles(bundleMappings);
                    done();
                    return pageBundles;
                }
            },
            
            getBundleMappingsCached: function(bundleSetConfig, enabledExtensions, async) {
                var cache = this.getCache(enabledExtensions);
                var cacheKey = async ? bundleSetConfig._id + '|async' : bundleSetConfig._id;

                var bundleMappings = cache.getBundleMappings(cacheKey);
                if (!bundleMappings) {
                    bundleMappings = this.config.createBundleMappings(bundleSetConfig, enabledExtensions, async);
                    
                    cache.addBundleMappings(bundleSetConfig._id, cacheKey);
                }
