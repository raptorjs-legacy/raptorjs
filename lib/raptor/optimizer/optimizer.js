...
define(
    ...
    function(raptor, require, exports, module) {
        ...
        var optimizer = {
            ...
            forEachDependencyAsync: function(options) {
                return this.forEachDependency(options, true);
            },
             ...
             * @param options
             */
            forEachDependency: function(options, async) {
                ...
                var enabledExtensions = options.enabledExtensions, 
                    ...
                    _this = this,
                    isDone = false,
                    deferred = async === true ? promises.defer() : null;
    
    
                var foundDependencies = {};
                
                function handleManifest(manifest, parentPackage, recursive, depth, async) {
                    ...
                    if (recursive === true || depth <= 0) {
                        ...
                        manifest.forEachDependency(
                            function(type, packageDependency) {
                                ...
                                handleDependency(packageDependency, manifest, recursive, depth+1, async || packageDependency.isAsync());
                            },
                            _this,
                            {
                ...
                function handleDependency(dependency, parentPackage, recursive, depth, async) {
                    ...
                    if (dependency.isPackageDependency()) {
                        if (dependency.isDeferred && dependency.isDeferred()) {
                            var promise = dependency.getPromise();
                            promise.then(
                                function(dependencyManifest) {
                                    handleManifest(dependencyManifest, parentPackage, recursive, depth, async);
                                });
                            return promise;
                        }
                        else {
                ...
                function handleDependencies(start) {

                    for (var i=start; i<dependencies.length; i++) {
                        var dependency = packaging.createDependency(dependencies[i]);

                        var promise = handleDependency(
                            dependency, 
                            null,
                ...
                if (async) {
                    ...
                    if (!isDone) {
                        throw new Error("Asynchronous forEachDependency required. Use forEachDependencyAsync() instead");
                    }
                    return null;
                }
            },
