define('raptor/optimizer/cli', function (require, exports, module) {
    /*jshint sub:true */
    'use strict';
    var files = require('raptor/files'), resources = require('raptor/resources'), optimizer = require('raptor/optimizer'), File = require('raptor/files/File'), endsWith = require('raptor/strings').endsWith, logger = module.logger(), cwd = process.cwd();
    return {
        run: function (args) {
            var pageName, packageManifest = null, packagePath, dependencies = [], sourceDirs, configPath, outputDirPath = null, minify, params, includeChecksums = false, urlPrefix = null, developmentMode = false, generateHtmlSlotsFile = false, enabledExtensions = null, basePath, parseDependencies = function (dependencies) {
            ...
            var argv = require('optimist')(args).usage('Usage: $0 [dependency-1] [dependency-2] [dependency-n] [options]\nExamples:\n' + '  Optimize a set of dependencies:\n' + '   $0 my-module /some-resource.js --name test-page --source=path/to/my-modules\n\n' + '  Optimize a package:\n' + '   $0 --name test-page --package path/to/package.json --source=path/to/my-modules\n\n' + '  Optimize using a configuration file:\n' + '   $0 --name test-page --package path/to/package.json --source=path/to/my-modules --config path/to/optimizer-config.xml\n\n' + '  Optimize all of the pages defined in a configuration file:\n' + '   $0 --config path/to/optimizer-config.xml\n' + '\n' + 'NOTE: Modules to download asynchronously should be suffixed with a question mark. For example:\n' + '  Optimize a page with an asynchronous dependency:\n' + '   $0 my-module my-async-module? --name test-page\n').alias('n', 'name').describe('n', 'The name of the page being optimized (e.g. "my-page")').alias('p', 'package').describe('p', 'A package manifest describing page dependencies').alias('o', 'out').describe('o', 'The output directory for static bundles and optimized page JSON files').alias('s', 'source').describe('s', 'A comma-separated list of source directories to search for modules and resources').alias('c', 'config').describe('c', 'Path to an optimizer XML configuration file').alias('m', 'minify').boolean('m').describe('m', 'Enable JavaScript and CSS minification (disabled by default)').describe('checksum', 'Include checksums for files').boolean('checksum').describe('help', 'Display this usage information').alias('u', 'url-prefix').describe('u', 'URL prefix for resource bundles').alias('d', 'development').describe('d', 'Enable developer-friendly mode').boolean('d').alias('b', 'base-path').describe('b', 'Base path for generating URLs').alias('h', 'html').describe('h', 'Generate a JSON file that contains the HTML markup required to include the dependencies (organized by slot)').alias('e', 'extensions').describe('e', 'Comma separated list of enabled extensions').boolean('html').check(function (argv) {
                    if (argv['help']) {
                        throw '';
                    }
                    if (args.length === 2) {
            ...
            if (dependencies.length) {
                packageManifest = require('raptor/packaging').createPackageManifest({ dependencies: dependencies }, resources.createFileResource(cwd));
            }
            ...
            require('raptor/promises').all(promises).then(function () {
