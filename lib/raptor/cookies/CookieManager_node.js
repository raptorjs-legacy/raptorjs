...
define.extend('raptor/cookies/CookieManager', function(require) {
    ...
    return {
        ...
        _commitCookie: function(name, value, domain, path, maxAge, secure, httpOnly) {
            var expires;
            var headerValueParts = [name + '=' + encodeURIComponent(value)];
        ...
        _doLoadCookies: function() {
            ...
            if (!cookies) {
                // If cookies have not already been parsed then
                // we will do that now by reading the cookie header
                var cookiesHeader = request.headers.cookie;
                if (cookiesHeader) {
                    cookies = nodeCookie.parse(cookiesHeader);    
                }
            }
            ...
            if (cookies) {
                ...
                raptor.forEachEntry(cookies, function(name, value) {
                    ...
                    this.addCookieInternal(name, value);
                }, this);
            }
        }
    };
