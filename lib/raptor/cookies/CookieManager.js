        ...
        commitCookies: function () {
            forEachEntry(this._cookies, function (name, cookie) {
                if (cookie.isModified()) {
                    var value, maxAge, domain = cookie.getDomain(), path = cookie.getPath(), secure = cookie.isSecure(), httpOnly = cookie.isHttpOnly();
                    if (cookie.isDeleted()) {
                        value = '';
                        maxAge = 0;
                    } else {
                    ...
                    this._commitCookie(name, value, domain, path, maxAge, secure, httpOnly);
                }
            }, this);
        },
