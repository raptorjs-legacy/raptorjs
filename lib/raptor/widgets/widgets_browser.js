...
define.extend('raptor/widgets', function (require, widgets) {
    'use strict';
    var logger = require('raptor/logging').logger('raptor/widgets'), widgetsById = {}, raptor = require('raptor'), isArray = Array.isArray, createError = raptor.createError, Widget = require('raptor/widgets/Widget'), _convertEvents = function (events) {
            var convertedEvents = {};
    ...
    var _registerWidget = function (type, id, assignedId, config, scope, events, parent, bubbleErrorsDisabled) {
        ...
        var widget,
            // This will be the newly created widget instance of the provided type
            OriginalWidgetClass = require(type);
        ...
        if (OriginalWidgetClass.initWidget) {
            ...
            if (!OriginalWidgetClass.onReady) {
                //Add an onReady function that can be used to initialize the widget onReady
            ...
            widget.widgets = new Document();
            //This widget might have other widgets scoped within it 
            widgetsById[id] = widget;
            // Register the widget in a global lookup
            if (assignedId && scope) {
                // If the widget is scoped within another widget then register the widget in the scope
                var isTargetArray;
                ...
                containingWidget.widgets._add(widget, assignedId, isTargetArray);
                if (Widget.legacy) {
    ...
    require('raptor/pubsub').subscribe({
        ...
        'raptor/renderer/renderedToDOM': function (eventArgs) {
            /*jshint strict:false */
            var context = eventArgs.context, widgetsContext = widgets.getWidgetsContext(context);
            widgetsContext.initWidgets();
        }
    });
    return {
        initWidget: function (widgetDef) {
            var result = _registerWidget(widgetDef.type, widgetDef.id, widgetDef.assignedId, widgetDef.config, widgetDef.scope ? widgetDef.scope.id : null, widgetDef.events);
        ...
        _serverInit: function (widgetDefs) {
            var _initWidgets = function (widgetDefs, parent) {
                ...
                var i = 0, len = widgetDefs.length;
                for (; i < len; i++) {
                    // Each widget def serialized from the server is encoded into a minimal
                    // array object that we need to decipher...
                    var widgetDef = widgetDefs[i], type = widgetDef[0], id = widgetDef[1], config = widgetDef[2] || {}, scope = widgetDef[3], assignedId = widgetDef[4], events = widgetDef[5] || {}, children = widgetDef.slice(6);
                    if (scope === 0) {
                        scope = undefined;
                    }
                    if (assignedId === 0) {
