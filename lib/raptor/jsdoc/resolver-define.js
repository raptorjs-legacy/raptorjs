...
module.exports = function (methodName, node, walker, isClass, isExtend, isEnum) {
    'use strict';
    var name = null, label = null, args = node['arguments'], modifiers = null, def = null, type = null, extension = null, isModule = false;
    if (node.comment) {
    ...
    args.forEach(function (arg, i) {
        if (arg.type === 'FunctionExpression') {
            ...
            if (i !== 0) {
                modifiers = { superclass: arg.value };
            } else {
                label = arg.value;
    ...
    if (!def) {
        ...
        if (name) {
            ...
            scope.forEachLocalVar(function (prop) {
                var varName = prop.name, varType = prop.type;
    ...
    if (type) {
        ...
        if (!type.isJavaScriptFunction() && isClass) {
             ...
             * class
             */
            var initProp = type.getProperty('init'), classType;
            if (initProp) {
