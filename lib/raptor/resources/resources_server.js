...
define.extend('raptor/resources', function (require) {
    ...
    var raptor = require('raptor'), strings = require('raptor/strings'), logger = require('raptor/logging').logger('raptor/resources'), DirSearchPathEntry = require('raptor/resources/DirSearchPathEntry'), Resource = require('raptor/resources/Resource'), MissingResource = require('raptor/resources/MissingResource'), FileResource = require('raptor/resources/FileResource'), SearchPath = require('raptor/resources/SearchPath'), searchPath = new SearchPath(), _createDirSearchPathEntry = function (config) {
            ...
            if (config.type == 'dir') {
                return searchPath.addEntry(_createDirSearchPathEntry(config));
            } else {
    ...
    return {
        ...
        addSearchPathEntry: function (searchPathEntry) {
            return searchPath.addEntry(searchPathEntry);
        },
        addSearchPathDir: function (path) {
            return _addSearchPathEntry({
                type: 'dir',
                path: path
            });
        },
        ...
        findResourcePreferredSearchPath: function (path, searchPathEntry) {
            if (!searchPathEntry) {
            ...
            if (!resource) {
                searchPath.forEachEntry(function (entry) {
                    if (entry === searchPathEntry) {
                        // Skip the search path entry we already checked
                        return;
                    }
                    resource = entry.findResource(path);
                    if (resource != null) {
        ...
        resolvePath: function (basePath, relativePath) {
            ...
            var parts = relativePath.split('/'), pathParts = basePath.split('/');
        ...
        resolveResource: function (baseResource, relPath) {
            ...
            if (strings.startsWith(relPath, '/')) {
                if (baseResource) {
                    resource = this.findResource(relPath, baseResource.getSearchPathEntry());
                }
            ...
            if (packageSearchPath != null) {
                ...
                raptor.forEach(packageSearchPath, function (searchPathConfig) {
                    if (typeof searchPathConfig === 'string') {
                        searchPathConfig = { dir: searchPathConfig };
                    }
