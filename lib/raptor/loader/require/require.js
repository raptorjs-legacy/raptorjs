...
define.extend('raptor/loader', function (require) {
    'use strict';
    var logger = require('raptor/logging').logger('raptor/loader'), raptor = require('raptor'), forEach = raptor.forEach, extend = raptor.extend;
    ...
    return {
        ...
        load: function (requires, callback, thisObj) {
            var userSuccessFunc,
                //A reference to the user's success callback (if provided)
                wrappedCallback, isFunctionCallback = typeof callback === 'function';
            ...
            return this.include({ requires: requires }, wrappedCallback || callback, thisObj);
        }
    };
});