...
define.extend('raptor/loader', function (require) {
    'use strict';
    var extend = require('raptor').extend, logger = require('raptor/logging').logger('raptor/loader');
    return {
        ...
        includeCSSImpl: function (href, callback, attributes) {
            ...
            if (document.createStyleSheet) {
                ...
                if ('loaded' === readyState || 'complete' === readyState) {
                    ...
                    var detachEvents, detached = false, addEventListener = document.addEventListener, onReadyStateChange = function () {
                            detachEvents();
                            var readyState = document.readyState;
                            if ('loaded' === readyState || 'complete' === readyState) {
                                success();
                    ...
                    detachEvents = function () {
                        if (!detached) {
                            success();
                            // Invoke the success callback before detaching... if already invoked then it will be a no-op
                            detached = true;
                            if (addEventListener) {
                                document.removeEventListener('load', onLoad);
                                document.removeEventListener('readystatechange', onReadyStateChange);
                    ...
                    window.setTimeout(detachEvents, 300);    // After 300ms, automatically detach events and invoke the success callback
                }
            } else {
                cleanup = function () {
                    el.onload = null;
