...
define('raptor/loader', ['raptor'], function (raptor, require, exports, module) {
    'use strict';
    var included = {}, downloaded = {}, forEach = raptor.forEach, forEachEntry = raptor.forEachEntry, listeners = require('raptor/listeners'), events = [
            'asyncStart',
            'asyncComplete',
            'success',
            'error',
            ...
            return observable;
        }, progressEvents = listeners.createObservable(), _handleUrlStart = function (url, callback) {
    ...
    var Transaction = function (loader) {
        var _includes = [], _included = {}, started, _this = {
                _add: function (url, include, includeFunc, thisObj) {
                    ...
                    _includes.push(function (callback) {
                        ...
                        includeFunc.call(thisObj, include, _createImplCallback(url, callback));
                    });
                },
                ...
                execute: function (userCallback) {
                    started = 1;
                    var failed = [], status = { failed: failed };
                    if (!_includes.length) {
                        userCallback.success(status);
                        userCallback.complete(status);
                    }
                    var completedCount = 0, asyncStarted = false, callback = _createAsyncCallback({
                            asyncStart: function () {
                                if (!asyncStarted) {
                                    asyncStarted = true;
