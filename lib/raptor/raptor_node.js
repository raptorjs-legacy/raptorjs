exports.load = function(raptor) {
    ...
    var defineExcludes = null;

    raptor.excludeDefine = function(filter, recursive) {
        if (!defineExcludes) {
            defineExcludes = require('path-filters').create();
        }
        
        defineExcludes.add(filter, recursive);
    }
    ...
    if (!global.raptorCreateDefine) {
        ...
        var raptorWrapper = " if (global && global.raptorCreateDefine && global.raptorIsDefineIncluded(module)) { define=global.raptorCreateDefine(module); } ",
        ...
        global.raptorIsDefineIncluded = function(module) {
            return defineExcludes ? !defineExcludes.hasMatch(module.filename) : true;
        }
    }
};
