...
define.extend('raptor/templating', function (require) {
    'use strict';
    var strings = require('raptor/strings'), resources = require('raptor/resources'), File = require('raptor/files/File'), templateInfoByName = {}, logger = require('raptor/logging').logger('raptor/templating_server');
    return {
        findTemplate: function (name) {
            var path = name, resource, templatePath, templateFile = name instanceof File ? name : new File(name);
