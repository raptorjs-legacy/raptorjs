...
define.extend('raptor/templating', function(require) {
    ...
    return {
        ...
        findTemplate: function(name) {
            ...
            if (resource && resource.exists()) {
                if (resource.isFileResource()) {
                    pathsForTemplateNames[name] = resource.getFilePath();
                }
        ...
        unloadFile: function(path) {
            for (var curName in pathsForTemplateNames) {
                if (pathsForTemplateNames.hasOwnProperty(curName)) {
                    var curPath = pathsForTemplateNames[curName];
                    if (curPath === path) {
                        this.unload(curName);    
                    }
                }
        ...
        renderToFile: function(templateName, outputFile, data, context) {
            if (typeof outputFile === 'string') {
                outputFile = new File(outputFile);
            }

            var html = this.renderToString(templateName, data, context);
