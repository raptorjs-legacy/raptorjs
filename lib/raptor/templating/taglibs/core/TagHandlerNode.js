 ...
 */
define.Class('raptor/templating/taglibs/core/TagHandlerNode', 'raptor/templating/compiler/Node', ['raptor'], function (raptor, require) {
    'use strict';
    var extend = raptor.extend, objects = require('raptor/objects'), forEach = raptor.forEach, forEachEntry = raptor.forEachEntry, stringify = require('raptor/json/stringify'), Expression = require('raptor/templating/compiler/Expression'), addHandlerVar = function (template, handlerClass) {
            ...
            _this.tag.forEachVariable(function (v) {
                ...
                if (v.nameFromAttribute) {
                    ...
                    for (var i = 0, len = possibleNameAttributes.length; i < len; i++) {
                        var attrName = possibleNameAttributes[i], keep = false;
