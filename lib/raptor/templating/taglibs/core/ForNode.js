 ...
 */
define.Class('raptor/templating/taglibs/core/ForNode', 'raptor/templating/compiler/Node', ['raptor'], function (raptor, require) {
    'use strict';
    var forEachRegEx = /^\s*([A-Za-z_][A-Za-z0-9_]*)\s+in\s+(.+)$/, forEachPropRegEx = /^\(\s*([A-Za-z_][A-Za-z0-9_]*)\s*,\s*([A-Za-z_][A-Za-z0-9_]*)\s*\)\s+in\s+(.+)$/, stringify = require('raptor/json/stringify').stringify, parseForEach = function (value) {
    ...
    ForNode.prototype = {
        doGenerateCode: function (template) {
            var each = this.getProperty('each'), separator = this.getProperty('separator'), statusVar = this.getProperty('statusVar') || this.getProperty('varStatus'), customIterator = this.getProperty('iterator');
            if (!each) {
            ...
            var items = template.makeExpression(parts['in']), varName = parts['var'], nameVarName = parts.nameVar, valueVarName = parts.valueVar;
            if (nameVarName) {
                if (separator) {
