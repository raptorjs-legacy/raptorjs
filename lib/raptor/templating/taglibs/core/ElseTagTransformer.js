 ...
 */
define.Class('raptor/templating/taglibs/core/ElseTagTransformer', ['raptor'], function (raptor, require) {
    'use strict';
    var strings = require('raptor/strings');
    return {
        process: function (node, compiler) {
            var curNode = node.previousSibling, matchingNode, IfNode = compiler.getNodeClass('http://raptorjs.org/templates/core', 'if'), ElseIfNode = compiler.getNodeClass('http://raptorjs.org/templates/core', 'else-if'), whitespaceNodes = [];
            while (curNode) {
