 ...
 */
define.Class('raptor/templating/taglibs/widgets/WidgetsTagTransformer', ['raptor'], function (raptor, require) {
    'use strict';
    var widgetsNS = 'http://raptorjs.org/templates/widgets', strings = require('raptor/strings'), objects = require('raptor/objects'), stringify = require('raptor/json/stringify'), AttributeSplitter = require('raptor/templating/compiler/AttributeSplitter');
    return {
        process: function (node, compiler, template) {
            var widgetAttr = node.getAttributeNS(widgetsNS, 'widget'), widgetElIdAttr, widgetProps = widgetAttr ? null : node.getPropertiesNS(widgetsNS);
            var widgetArgs = {}, widgetEvents = [];
            if (widgetProps) {
            ...
            if (widgetAttr) {
                ...
                var widgetJsClass = compiler.convertType(widgetAttr, 'string', true), config, assignedId;
