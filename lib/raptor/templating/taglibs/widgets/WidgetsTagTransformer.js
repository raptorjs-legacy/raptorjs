...
define.Class(
    'raptor/templating/taglibs/widgets/WidgetsTagTransformer',
    ['raptor'],
...
//                            
                    ...
                    if (!objects.isEmpty(widgetArgs)) {
                        ...
                        node.addPreInvokeCode(template.makeExpression(
                            "_widgetArgs(" + 
                                widgetArgsParts.join(", ") +
                            ")"
                        ));
                ...
                if ((widgetAttr = node.getAttributeNS(widgetsNS, "widget"))) {
                    ...
                    var widgetNodeProps = AttributeSplitter.parse(
                        widgetAttr, 
                        {
                            jsClass: {
                                type: "custom"
                        ...
                        },
                        {
                            ...
                            removeDashes: true,
                            errorHandler: function(message) {
                                node.addError('Invalid c:widget attribute of "' + widgetAttr + '". Error: ' + message);
                            }
                        });

                    var widgetJsClass = widgetNodeProps.jsClass,
                        config = widgetNodeProps.config,
                        assignedId = widgetNodeProps.id;
