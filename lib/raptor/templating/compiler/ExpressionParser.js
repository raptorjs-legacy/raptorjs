 ...
 */
define.Class('raptor/templating/compiler/ExpressionParser', ['raptor'], function (raptor, require) {
    'use strict';
    var Expression = require('raptor/templating/compiler/Expression'), strings = require('raptor/strings'), stringify = require('raptor/json/stringify'), regexp = require('raptor/regexp'), endingTokens = {
            '${': '}',
            '$!{': '}',
            '{%': '%}',
            '{?': '}',
            ...
            pos = line.pos;
            str = line.str;
            var start = pos - length, end = pos + length, i;
            if (start < 0) {
                start = 0;
            }
            if (end > str.length) {
            ...
            var tokensRegExp = /"(?:[^"]|\\")*"|'(?:[^']|\\')*'|\\\\;|\\;|[\{\};]/g, matches, depth = 0;
            var parts = [], partStart = 0;
            ...
            var hasExpression, parts, handleText = function (text) {
    ...
    ExpressionParser.parse = function (str, callback, thisObj, options) {
        ...
        var textStart = 0,
            //The index of the start of the next text block
            textEnd,
            //The index of the current text block
            startMatches,
            //The matches found when searching for the possible start tokens
            endMatches,
            //The matches found when searching through special expression tokens
            expressionStart,
            //The index of the start of the current expression
            expression,
            //The current expression string
            isScriptlet,
            //If true, then the expression is a scriptlet,
            isConditional,
            //If true, then the expression is a conditional (i.e. {?<expression>;<true-template>[;<false-template>]}
            startToken,
            //The start token for the current expression
            custom = options.custom || {}, handleError = function (message) {
                if (callback.error) {
        ...
        outer:
            while (startMatches = startRegExp.exec(str)) {
                ...
                if (!endToken) {
                    //Check if the start token has an end token... not all start tokens do. For example: $myVar
                ...
                //Start searching from where the expression begins
                var depth = 0;
                var foundStrings = [];
                var handler;
                while (endMatches = endRegExp.exec(str)) {
                    ...
                    handler = null;
                    if (startToken === '${') {
                        var firstColon = expression.indexOf(':'), customType;
                        if (firstColon != -1) {
