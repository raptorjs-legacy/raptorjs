 ...
 */
define.Class('raptor/templating/compiler/TaglibCollection', ['raptor'], function (raptor, require, exports, module) {
    'use strict';
    var forEach = raptor.forEach, strings = require('raptor/strings'), ElementNode = require('raptor/templating/compiler/ElementNode'), TextNode = require('raptor/templating/compiler/TextNode'),
        /*
         ...
         * core as my-core
         */
        importRegExp = /^(?:(\*|(?:(?:@?[A-Za-z0-9_\-]+\s*,\s*)*@?[A-Za-z0-9_\-]+))\s+from\s+)?([^ ]*)(?:\s+as\s+([A-Za-z0-9_\-]+))?$/, getImported = function (lookup, localName, imports) {
            if (lookup[localName] != null) {
                return lookup[localName];
            }
            var prefixEnd = localName.indexOf('-'), prefix, uri, name;
            if (prefixEnd != -1) {
    ...
    TaglibCollection.prototype = {
        ...
        add: function (taglib) {
            ...
            taglib.forEachTag(function (tag, i) {
                var uri = tag.uri == null ? tag.uri = taglib.uri : tag.uri,
                    //If not specified, the tag URI should be the same as the taglib URI
                    name = tag.name, key = uri + ':' + name;
                ...
                if (tag.hasTransformers()) {
                    //Check if the tag has any transformers that should be applied
        ...
        forEachTagTransformer: function (uri, tagName, callback, thisObj) {
            ...
            //All transformers that match the URI and tag name exactly
