 ...
 */
define.Class('raptor/render-context/Context', ['raptor'], function (raptor, require) {
    'use strict';
    var forEachEntry = raptor.forEachEntry, escapeXmlAttr = require('raptor/xml/utils').escapeXmlAttr, StringBuilder = require('raptor/strings/StringBuilder'), createError = raptor.createError, nextUniqueId = 0, _classFunc = function (className, name) {
            var Clazz = require(className), func = Clazz[name] || Clazz.prototype && Clazz.prototype[name];
            if (!func) {
    ...
    var proto = {
            ...
            getFunction: function (className, name) {
                ...
                var key = className + ':' + name, helper = this._helpers[key];
                if (!helper) {
