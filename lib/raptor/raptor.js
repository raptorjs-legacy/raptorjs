(function() {
    var raptor, //The "raptor" module being created
        ...
        _makeClass = function(clazz, superclass) {
            if (!isFunction(clazz)) {
                var o = clazz;
        ...
        requireProps = {
            load: function(dependencies, callback) {
                ...
                return _require('raptor/loader').load(dependencies, callback);
            },
        ...
        _define = function(args, simpleRequire, isClass, isExtend, isEnum) {
            var i=0,
                ...
                require = _extendRequire(function(requestedId) { //This is the "require" function that the user code will see...Need to add the required props
                    return simpleRequire(requestedId, id); //Pass along the requested ID and the base ID to the require implementation
                }),
            ...
            if (isExtend) { //If define.extend then we need to register a "post create" function to modify the target module
                ...
                if (isClass || superclass) {
                    postCreate = function(instance) {
                        var clazz = _makeClass(instance, superclass);
                        return clazz;  
                    };
                }
                else if (isEnum) {
                    ...
                    postCreate = function(enumClass) {
                        if (enumClass) {
                            if (typeof enumClass == 'object') {
                                enumClass = _makeClass(enumClass); // Convert the class object definition to
    ...
    Module.prototype = {
        logger: function() {
            return _require('raptor/logging').logger(this.id);
        }
    };
    ...
    if (typeof exports == 'undefined') {
        ...
        require = _extendRequire(require);
