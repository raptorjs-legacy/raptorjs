define.extend('raptor/i18n', function(require, exports, module) {
    ...
    function I18nResource(resourcePath) {
        ...
        if (!this._dictionaryName) {
            ...
            this._dictionaryName = (this._fileBasePath.charAt(0) === '/') ? this._fileBasePath.substring(1) : base;
        }
    ...
    return {
        ...
        toI18nResource: function(obj) {
            if (obj.constructor === String) {
                return this.createI18nResource(obj);
            } else {
                return obj;
            }
        },
        ...
        _readRawDictionary: function(i18nResource, localeCode, callback, thisObj) {
            ...
            try {
                ...
                delete e.stack;
                logger.error('Error parsing JSON dictionary "' + resource.getPath() + '".', e);
                // store reference to resource in error object
                e.resource = resource;

                // invoke callback with error
                return callback.call(thisObj || this, e);
            }
        ...
        getDictionaries: function(localeCode) {
        ...
        loadDictionariesForModule: function(moduleName, localeCode, callback, thisObj) {
            this._findI18nDependencies(moduleName, function(err, dependencies) {
                ...
                function onLoadDictionary(err) {
                    if (err) {
                        logger.error('Error loading dictionaries for module "' + moduleName + '" and locale "' + localeCode + '".', err);
                    }
                    pending--;

                    if (pending === 0) {
                        logger.info('Finished loading dictionaries for module "' + moduleName + '" and locale "' + localeCode + '".');
                        callback.call(thisObj || this, null, this.getDictionaries(localeCode));
                    }
                }

                for (var i = 0; i < dependencies.length; i++) {
                    this.loadDictionary(dependencies[i].getI18nResource(), localeCode, onLoadDictionary, this);
                }
            }, this);
        },

        _getDictionary: function(i18nResource, localeCode) {
            var dictionaries = this.getDictionaries(localeCode),
                dictionaryName = i18nResource.getDictionaryName();

            return dictionaries.getDictionaryIfExists(dictionaryName);
        },
        ...
        _loadDictionary: function(i18nResource, localeCode, parentDictionary, callback, thisObj) {
            var dictionary = this._getDictionary(i18nResource, localeCode);
            if (dictionary) {
                logger.info('Found cached dictionary "' + i18nResource.getDictionaryName() + '" for locale "' + localeCode + '".');
                return callback.call(thisObj || this, null, dictionary);
            }

            logger.info('Loading dictionary "' + i18nResource.getDictionaryName() + '" for locale "' + localeCode + '"...');

            this._readRawDictionary(i18nResource, localeCode, function(err, dictionaryObj) {
                if (err) {
                ...
                if (dictionaryObj) {
                    ...
                    this.compileDictionary(i18nResource.getDictionaryName(), localeCode, dictionaryObj, writer);
                    ...
                    if (writer.code.length > 0) {
                        try {
                            ...
                            var resourcePath = i18nResource.getLocalizedResourcePath(localeCode),
                                message = 'Unable to load compiled dictionary "' + resourcePath + '". Exception: ' + e,
                                err = raptor.createError(new Error(message), e);
                    ...
                    logger.info('No localized dictionary for "' + i18nResource.getDictionaryName() + '" for locale "' + localeCode + '" found.');
                ...
                var Dictionary = require('raptor/i18n/Dictionary'),
                    dictionary = new Dictionary(i18nResource.getDictionaryName(), dictionaryObj, localeCode);

                this.getDictionaries(localeCode).setDictionary(i18nResource.getDictionaryName(), dictionary);

                logger.info('Loaded dictionary "' + i18nResource.getDictionaryName() + '" for locale "' + localeCode + '".');
                callback.call(thisObj || this, null, dictionary);
        ...
        loadDictionary: function(i18nResource, localeCode, callback, thisObj) {
            ...
            if (!localeCode) {
                // there's no parent dictionary so just load the dictionary for the default locale
                return this._loadDictionary(i18nResource, localeCode, null /* parentDictionary */, callback, thisObj)
            }

            // first load the default dictionary
            this._loadDictionary(i18nResource, '', null /* parentDictionary */, function(err, parentDictionary) {
                if (err) {
                    callback.call(thisObj || this, err);
                    return;
                }
                ...
                if ((localeCode.charAt(2) === '_') && this.isLocaleSupported(subLocaleCode = localeCode.substring(0, 2))) {
                    this._loadDictionary(i18nResource, subLocaleCode, parentDictionary, function(err, parentDictionary) {
                        if (err) {
                            callback.call(thisObj || this, err);
                            return;
                        }
                        ...
                        this._loadDictionary(i18nResource, localeCode, parentDictionary, callback, thisObj)
                    });
                } else {
                    // We're done loading all of the parent dictionaries so load the final dictionary
                    this._loadDictionary(i18nResource, localeCode, parentDictionary, callback, thisObj)
        ...
        resolveDictionary: function(name, localeCode) {
            // loaded dictionaries are automatically put into Dictionaries
            // so if it is not there then it hasn't been loaded and that
            // will cause an error to be thrown by design
            return null;
        },
