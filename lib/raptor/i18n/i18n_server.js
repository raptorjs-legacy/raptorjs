define.extend('raptor/i18n', function (require, exports, module) {
    ...
    var logger = require('raptor/logging').logger('raptor/i18n');
    // cache of dictionaries by their locale code
    var dictionariesByLocaleCodeCache = require('raptor/caching').getDefaultProvider().getCache('raptor/i18n.dictionariesByLocaleCode');
    var specialTypes = {
            ...
            'template': function (key, value, localeCode, dictionary, dictionaryName, writer) {
                ...
                var dust = require('dustjs-linkedin'), templateName = dictionaryName + '_' + localeCode + '.' + key;
    ...
    function I18nResource(resourcePath) {
        ...
        var slashPos = resourcePath.lastIndexOf('/'), pos = resourcePath.indexOf('.', slashPos === -1 ? 0 : slashPos + 1);
        if (pos === -1) {
    ...
    return {
        ...
        _findI18nDependencies: function (moduleName, callback, thisObj) {
            var walker = packaging.createDependencyWalker(), dependencies = [], Dependency_i18n = require('raptor/packaging/Dependency_i18n');
            logger.info('Searching for i18n dependencies for module "' + moduleName + '"...');
            walker.walkModule(moduleName).onDependency(function (dependency, extension) {
        ...
        _loadDictionaryHelper: function (i18nResource, localeCode, parentDictionary) {
            var dictionaries = this.getDictionariesForLocale(localeCode), dictionaryName = i18nResource.getDictionaryName();
        ...
        loadDictionary: function (i18nResource, localeCode, callback, thisObj) {
            ...
            if (localeCode) {
                ...
                parentDictionary = this._loadDictionaryHelper(i18nResource, '', null);
        ...
        compileDictionary: function (name, localeCode, dictionary, writer) {
            ...
            for (var i = 0; i < keys.length; i++) {
                var key = keys[i], value = dictionary[key], type;
                if (value && value.type) {
                    type = specialTypes;
                }
        ...
        renderDustTemplate: function (templateName, input) {
            var dust = require('dustjs-linkedin'), _out;
