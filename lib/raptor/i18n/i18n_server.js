define.extend('raptor/i18n', function(require, exports, module) {
    ...
    return {
        ...
        readI18nResource: function(i18nResource, localeCode, callback, thisObj) {
            ...
            if (!resource.exists()) {
                logger.info('Dictionary "' + resourcePath + '" not found!!!');
                return null;
            }
            ...
            try {
                ...
                delete e.stack;
                e = raptor.createError('Error parsing JSON dictionary "' + resource.getPath() + '".', e);
                // store reference to resource in error object
                e.resource = i18nResource;
                throw e;
            }
            ...
            if (dictionaryObj) {
                ...
                if (writer.code.length > 0) {
                    try {
                        ...
                        logger.error('Unable to load compiled dictionary "' + resourcePath + '".', e);
                    }
                }
