...
define.Class('raptor/i18n/Dictionaries', function(require, exports, module) {
    ...
    Dictionaries.prototype = {
        ...
        getDictionaryIfExists: function(name) {
            var dictionary = this._dictionaryByName[name];
            if (!dictionary) {
                var dictionary = i18n.resolveDictionary(name, this._localeCode);
                if (!dictionary) {
                    return null;
                }
        ...
        getDictionary: function(name) {
            var dictionary = this.getDictionaryIfExists(name);
            if (!dictionary) {
                throw new Error('Dictionary "' + name + '" not loaded for locale "' + this._localeCode + '". Loaded dictionaries: ' + Object.keys(this._dictionaryByName).join(', '));
            }
            return dictionary;
        },
