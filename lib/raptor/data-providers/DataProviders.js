 ...
 */
define.Class('raptor/data-providers/DataProviders', ['raptor'], function (raptor, require) {
    'use strict';
    var forEachEntry = raptor.forEachEntry, promises = require('raptor/promises');
    ...
    DataProviders.prototype = {
        ...
        hasProvider: function (name) {
            return this.providers.hasOwnProperty(name) || this.parent && this.parent.hasProvider(name);
        },
        ...
        requestData: function (name, args) {
            var provider = typeof name === 'string' ? this.getProvider(name) : name;
            if (!provider) {
                throw raptor.createError(new Error('Data provider not found for "' + name + '"'));
            }
            var func = provider[0], thisObj = provider[1];
            ...
            try {
                ...
                if (!promises.isPromise(promise)) {
                    ...
                    deferred.promise.resolvedValue = value;
