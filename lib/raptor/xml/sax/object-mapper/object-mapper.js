 ...
 */
define('raptor/xml/sax/object-mapper', ['raptor'], function (raptor, require, exports, module) {
    'use strict';
    var sax = require('raptor/xml/sax'), forEachEntry = raptor.forEachEntry, arrays = require('raptor/arrays'), strings = require('raptor/strings'), STRING = 'string', BOOLEAN = 'boolean', OBJECT = 'object', NUMBER = 'number', INTEGER = 'integer', INT = 'int', getSchema = function (el, nodeType, parentSchema) {
            ...
            var uri = el.getNamespaceURI(), schema;
            if (uri) {
    ...
    Reader.prototype = {
        _parseProp: function (value, context) {
            ...
            if (parsePropFunc) {
                //If a property parser is provide then invoke that to get the actual text
        ...
        _setProperty: function (el, schema, targetObject, value, context) {
            ...
            if (typeof value === 'string') {
                if (this.trimText) {
                    //Trim the text if that option is enabled
        ...
        read: function (xmlSrc, filePath) {
            ...
            this.contextStack = [];
            this.skipping = false;
            var _this = this, rootObject, saxParser = this.saxParser;
            ...
            saxParser.on({
                ...
                startElement: function (el) {
                    var parentContext = _this.getCurrentContext(), parentSchema = parentContext ? parentContext.schema : _this.schema, curSchema;
                    ...
                    if (!_this.skipping) {
                        ...
                        for (var i = 0, len = attrs.length, attr; i < len; i++) {
                            ...
                            _this._setProperty(attr, attrSchema, context.object, attr.getValue(), attrContext);    //The context for the attribute
                        }
                    }
                },
                ...
                endElement: function () {
                    var context = _this.getCurrentContext(), parentContext = context.parentContext, curSchema = context.schema;
                    if (_this.skipping !== true) {
                        if (curSchema._type === STRING || curSchema._type === BOOLEAN) {
