define('raptor/css-parser', function (require) {
    'use strict';
    var promises = require('raptor/promises');
    return {
        findUrls: function (code, callback, thisObj) {
            var urlRegExp = /url\(\s*"([^\)]*)"\s*\)|url\(\s*'([^\)]*)'\s*\)|url\(([^\)]*)\)/g, matches;
        ...
        replaceUrls: function (code, callback, thisObj) {
            ...
            function handleReplacementPromise(match, replacementPromise) {
                replacementPromise.then(function (replacement) {
            ...
            function applyReplacements() {
                ...
                for (var i = matches.length - 1; i >= 0; i--) {
                    var match = matches[i], start = match.start, end = match.end, replacement = match.replacement;
                    if (replacement != null) {
            ...
            if (promiseArray.length) {
                ...
                promises.all(promiseArray).then(function () {
                    try {
                        applyReplacements();
                    } catch (e) {
                        deferred.reject(e);
