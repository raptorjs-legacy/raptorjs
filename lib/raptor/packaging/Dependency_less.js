            ...
            getCode: function(context) {
                ...
                try
                {
                    ...
                    parser.parse(lessSource, function (e, root) {
                        if (e) {
                            throw raptor.createError(new Error('Unable to parse Less file at path "' + resource.getURL() + '". Exception: ' + e.message));
                        } else {
                            try {
                                result = root.toCSS();    
                            }
                            catch(e) {
                                throw raptor.createError(new Error('Unable to generate CSS code for Less file at path "' + resource.getURL() + '". Exception: ' + e.message));    
                            }
                        }
                    });
                }
