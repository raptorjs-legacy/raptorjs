 ...
 */
define('raptor/packaging/PackageLoader', function (require) {
    'use strict';
    var loaded = {};
    ...
    PackageLoader.prototype = {
        load: function (resourcePath, options) {
            var manifest = resourcePath._isPackageManifest ? resourcePath : require('raptor/packaging').getPackageManifest(resourcePath), path = manifest.getPackageResource().getURL(), enabledExtensions = options.enabledExtensions;
            if (loaded[path] === true) {
                return;
            }
            loaded[path] = true;
