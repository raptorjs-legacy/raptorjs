 ...
 */
define.Class('raptor/packaging/Dependency', ['raptor'], function (raptor, require) {
    'use strict';
    var runtime = require('raptor/runtime');
    var Dependency = function () {
        this.properties = {};
        this.addProperty('recursive', { type: 'boolean' });
        this.addProperty('inline', { type: 'boolean' });
    };
    Dependency.prototype = {
        ...
        resolveResource: function (path, context) {
            var relative = path.charAt(0) !== '/', manifest = this.getParentManifest(), resource;
            if (relative) {
                if (!manifest) {
                    throw raptor.createError(new Error('Unable to resolve path  \'' + path + '" for dependency. Manifest not provided'));
                }
                resource = manifest.resolveResource(path);
                ...
                resource = require('raptor/resources').findResource(path);
                if (!resource || !resource.exists()) {
                    resource = require('raptor/resources').createFileResource(path);
                    if (!resource || !resource.exists()) {
        ...
        load: function (context) {
            ...
            var resource = this.getResource(), path = resource.getURL();
