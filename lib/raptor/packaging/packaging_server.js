...
define('raptor/packaging', ['raptor'], function (raptor, require, exports, module) {
    'use strict';
    var forEachEntry = raptor.forEachEntry, mappings = {}, logger = require('raptor/logging').logger('packaging-server'), packageManifests = {}, enabledExtensions = null, dependencyClasses = {}, discoveryComplete = false, searchPathListenerHandle = null, ExtensionCollection = require('raptor/packaging/ExtensionCollection'), PackageManifest = require('raptor/packaging/PackageManifest'), PackageLoader = require('raptor/packaging/PackageLoader'), getEnabledExtensions = function () {
            ...
            manifest.setDependencies([{ path: resource.getName() }]);
            return manifest;
    ...
    return {
        ...
        getPackageManifest: function (resourcePath, exists) {
            ...
            if (manifest === undefined) {
                var packageJson = packageResource.readAsString('UTF-8'), loadedManifest;
                try {
        ...
        getPackageManifestResourceForModule: function (name) {
            this._doDiscovery();
            var target = mappings[name], packaging = this, resource;
            if (target) {
                if (require('raptor/strings').endsWith(target, 'package.json')) {
                    resource = packaging.getPackageManifestResource(target);
            ...
            if (!name.endsWith('.js') && !name.endsWith('/js')) {
                var resources = require('raptor/resources');
                var basePath = '/' + name.replace(/\./g, '/');
                //sample basePath: /my-module/sub-module
                var resourcePath = basePath + '.js';
                //sample resourcePath: /my-module/sub-module.js
                ...
                if (lastSlash !== -1) {
                    resourcePath = basePath + '/' + basePath.substring(lastSlash + 1) + '.js';
                    //sample resourcePath: /my-module/sub-module/sub-module.js
                    resource = resources.findResourcePreferredSearchPath(resourcePath, searchPath);
                    if (resource.exists()) {
                        manifest = createModuleManifestForResource(resource);
                        // cache the "fake" package manifest that we created
        ...
        getModuleManifest: function (name) {
            ...
            var manifest = this.getPackageManifest(resource, true);
            if (manifest) {
