...
define('raptor/packaging/PackageManifest', ['raptor'], function (raptor, require, exports, module) {
     ...
     */
    var forEach = raptor.forEach, forEachEntry = raptor.forEachEntry, ExtensionCollection = require('raptor/packaging/ExtensionCollection'), nextId = 0, _packaging;
    ...
    var inferDependencyTypeFromPath = function (dependencyConfig, packaging) {
        ...
        // b) "3.js"
        // c) "js"
        var path = dependencyConfig.path, lastSlash = path.lastIndexOf('/'), dotPos;
        if (lastSlash === -1) {
    ...
    var normalizeDependencyConfig = function (dependencyConfig) {
        ...
        if (typeof dependencyConfig === 'string') {
            dependencyConfig = { path: dependencyConfig };
        }
    ...
    function convertGlobPatternsToGlobs(patterns, cache, minimatch) {
        for (var i = 0; i < patterns.length; i++) {
            var globPattern = patterns[i], mm = cache.get(globPattern);
            if (!mm) {
    ...
    function addDependency(dependency, target, manifest) {
        if (dependency.type === 'glob') {
            if (dependency.include !== undefined) {
                ...
                var includes = Array.isArray(dependency.include) ? dependency.include : [dependency.include], excludes;
    ...
    PackageManifest.prototype = {
        ...
        forEachDependency: function (options) {
            ...
            var dependencyFilter = options.dependencyFilter, callback = options.callback, thisObj = options.thisObj, _isExtensionEnabled = function (extensionName) {
                    if (enabledExtensions) {
                        var extensionParts = extensionName.split(/[,|]/), i = 0, len = extensionParts.length;
                        for (; i < len; i++) {
                            if (extensionParts[i] === '') {
