...
JSDOC.DocTag.prototype.nibbleType = function(src) {
	...
	if (src.match(/^\s*\{/)) {
		...
		var type = src.substring(typeRange[0]+1, typeRange[1]).trim()
		if (type.charAt(0) == '@') return src;
		
		this.type = type;
		//Breaks generics with comma separated types: this.type = this.type.replace(/\s*,\s*/g, "|"); // multiples can be separated by , or |
